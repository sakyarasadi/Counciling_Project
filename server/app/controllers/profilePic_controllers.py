...
propic_bp = Blueprint('propic_bp', __name__)

@propic_bp.route("/api/propic", methods=["POST"])
def upload_file():
    ...
    try:
        ...
        blobs = list(bucket.list_blobs(prefix=f"propic_files/{email}/"))
        existing_versions = [
        ...
        blob = bucket.blob(f"propic_files/{email}/{filename}")
...
@propic_bp.route("/api/propic", methods=["GET"])
